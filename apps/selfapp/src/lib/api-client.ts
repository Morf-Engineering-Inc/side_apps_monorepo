 ...
 * Create a new entry
 */
export async function createEntry(entry: Omit<Entry, 'userId' | 'entryId' | 'createdAt' | 'updatedAt'>): Promise<Entry> {
  const response = await apiRequest<{ success: boolean; entry: Entry }>(
    '/api/entries',
    {
      method: 'POST',
...
export async function updateEntry(
  entryId: string,
  updates: Partial<Omit<Entry, 'userId' | 'entryId' | 'createdAt' | 'updatedAt'>>
): Promise<Entry> {
  const response = await apiRequest<{ success: boolean; entry: Entry }>(
    `/api/entries/${entryId}`,
    {
